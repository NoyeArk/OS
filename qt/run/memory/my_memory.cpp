 ...
 * \return 
 */
std::pair<int, int> Memory::LRU(const std::vector<std::shared_ptr<PCB>>& pcbs) {
    ...
    return std::make_pair(pid, LRUBlockId);
}
...
std::vector<std::pair<int, int>> Memory::PagesReplace(const std::vector<std::shared_ptr<PCB>>& pcbs, int pageNumToReplace) {
    std::vector<std::pair<int, int>> replaceBlocksId;  // 需要被置换到外存的内存块号
	for (size_t ii = 0; ii < pageNumToReplace; ii++) {
        std::pair<int, int> lruBlockId = this->LRU(pcbs);
