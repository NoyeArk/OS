...
private:
    ...
    std::pair<int, int> LRU(const std::vector<std::shared_ptr<PCB>>& pcbs);
...
public:
    ...
    std::vector<std::pair<int, int>> PagesReplace(const std::vector<std::shared_ptr<PCB>>& pcbs, int pageNumToReplace);
};
